...
OpenHDRC::OpenHDRC(QObject *parent): QObject(parent) {
    ...
    connect(jinstance, &QJoysticks::buttonChanged, this, &OpenHDRC::buttonChanged);
#endif
...
void OpenHDRC::channelTrigger() {
	...
	rcChannels[8] = btn_map[0] & 0xFF;
	rcChannels[9] = (btn_map[0] >> 8) & 0xFF;

	rcChannels[10] = btn_map[1] & 0xFF;
	rcChannels[11] = (btn_map[1] >> 8) & 0xFF;

	rcChannels[12] = btn_map[2] & 0xFF;
	rcChannels[13] = (btn_map[2] >> 8) & 0xFF;

	rcChannels[14] = btn_map[3] & 0xFF;
	rcChannels[15] = (btn_map[3] >> 8) & 0xFF;
